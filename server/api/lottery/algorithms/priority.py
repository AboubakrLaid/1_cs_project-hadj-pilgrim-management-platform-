...
def select_winners(participants_ids, available_seats, process, status, result_list):
    winners_count = 0
    backup = []
    while participants_ids and available_seats > 0:
        ...
        if participant_status.process != UserStatus.Process.VISIT.value:
            ...
            if participant.gender == "M" or (available_seats > 1):
                print(f"winner: {winner}, gender: {participant.gender}, available_seats: {available_seats}, participants_ids: {participants_ids}")
